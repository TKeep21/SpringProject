...
Схемы архитектуры, флоу отчетов и модели данных: [docs/architecture-diagrams.md](docs/architecture-diagrams.md).
...
Запустить demo UI после старта backend-сервисов:
...
Frontend нужно открывать именно с `http://localhost:3000`, потому что этот origin разрешен в CORS-конфигурации сервисов.
...
При добавлении участника тело запроса принимает `email` и `role`; сервис сам находит пользователя через `auth-service` и сохраняет его UUID в составе группы.
